...
| `run/run.py` | 默认启用同一根节点修复（Exact / QAIA / CIM 都启用），20 次尝试、最多 2 秒；新增 `--primal-completion 0|1` 作同代码消融。仓库现有本地密钥填写方式未动。 |
...
```powershell
python -m run.run --out-dir results/primal_repair_v1 --limit 600 --seeds 0 1 2 --exact-repeats 3
```
